...
(unlike examples/recipe.py's dict-based ``ingredients``) - that's what lets follow.design's
...
class CakeTrialBatch(Structure):
    ...
    follow.batch/follow.design - a batch is still one intent, one protocol, one conclusion, even
